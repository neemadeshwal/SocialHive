...
export default function MyPost() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("/api/newpost");
  ...
  const clickedPhoto = data.post.filter((post: any) => post._id == clickId);
  console.log(data);
  return (
    <div className="mt-10 grid grid-cols-3 gap-2 grid-flow-row transition ease-in-out duration-700">
      {data.post.map((post: any) => {
        ...
        return (
          <div
            ...
            style={{
              filter: `${
                onHover && hoverId === _id ? "brightness(0.7)" : "brightness(1)"
              }`,
            }}
          >
            <img
